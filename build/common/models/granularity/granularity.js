...
function getBucketSize(input) {
    if (input instanceof plywood_1.TimeBucketExpression)
        return input.duration.getCanonicalLength();
    if (input instanceof plywood_1.NumberBucketExpression)
        return input.size;
...
function bucketUnitToGranularity(input) {
    if (input instanceof chronoshift_1.Duration) {
        return new plywood_1.TimeBucketExpression({ duration: input });
    }
    else if (!isNaN(input)) {
        return new plywood_1.NumberBucketExpression({ size: input, offset: 0 });
    }
...
function granularityFromJS(input) {
    if (typeof input === "number")
        return plywood_1.NumberBucketExpression.fromJS({ size: input });
    if (typeof input === "string")
        return plywood_1.TimeBucketExpression.fromJS({ duration: input });
    if (typeof input === "object") {
        if (!general_1.hasOwnProperty(input, "op")) {
            throw new Error("could not recognize object as expression");
        }
        return plywood_1.Expression.fromJS(input);
    }
...
function granularityToString(input) {
    ...
    else if (input instanceof plywood_1.NumberBucketExpression) {
...
function updateBucketSize(existing, newInput) {
    if (newInput instanceof plywood_1.TimeBucketExpression) {
        return new plywood_1.TimeBucketExpression({
            duration: newInput.duration,
            timezone: existing.timezone
        });
    }
    else if (newInput instanceof plywood_1.NumberBucketExpression) {
        var value = { size: newInput.size };
        if (existing.offset)
            value.offset = existing.offset;
        return new plywood_1.NumberBucketExpression(value);
    }
